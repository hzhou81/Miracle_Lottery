...
def create_data(data, name, windows):
    ...
    cut_num = model_args[name]["model_args"]["sequence_len"] if name != "dlt" else model_args[name]["model_args"]["red_sequence_len"]
    return {
        "red": {
...
def train_with_eval_red_ball_model(name, x_train, y_train, x_test, y_test):
    ...
    with tf.compat.v1.Session() as sess:
        red_ball_model = LstmWithCRFModel(
            ...
            ball_num=m_args["model_args"]["sequence_len"] if name != "dlt" else m_args["model_args"]["red_sequence_len"],
        ...
        sequence_len = m_args["model_args"]["sequence_len"] \
            if name != "dlt" else m_args["model_args"]["red_sequence_len"]
...
def train_with_eval_blue_ball_model(name, x_train, y_train, x_test, y_test):
    ...
    if name == "ssq" or name == "qlc":
        ...
        sess.run(tf.compat.v1.global_variables_initializer())
        sequence_len = "" if name == "ssq" or name == "qlc" else m_args["model_args"]["blue_sequence_len"]
        ...
        logger.info("训练耗时: {}".format(time.time() - start_time))
        pred_key[ball_name[1][0]] = blue_ball_model.pred_label.name if (name == "ssq" or name == "qlc") else blue_ball_model.pred_sequence.name
